...
export class AuthService {
  ...
  async createAccessToken(userId: string) {
    const accessToken = sign({ userId }, process.env.JWT_SECRET, { expiresIn: parseInt(process.env.JWT_EXPIRATION) });
    return this.encryptText(accessToken);
  }

  async createRefreshToken(req: Request, userId) {
    var refreshToken = await this.refreshTokenModel.findOne({ userId: userId });
    if (refreshToken) {
      refreshToken.refreshToken = v4()
      refreshToken.ip = this.getIp(req)
      refreshToken.browser = this.getBrowserInfo(req)
      refreshToken.country = this.getCountry(req)
    } else {
